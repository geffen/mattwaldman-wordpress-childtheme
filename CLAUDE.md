...
- Files are `.dc.html` "Design Component" prototypes (streaming HTML format, not production code — treat rendered layout/color/copy as the source of truth, not the literal markup). Full file list: `Header 1C.dc.html` (canonical header/footer chrome, done), `Article Template.dc.html` (up next), `Resources Page.dc.html`, `About Page.dc.html`, `Buy the RSP.dc.html`, `Member Login.dc.html`, `Site Index.dc.html` (dev-only, not for production). Images in `uploads/`.
...
- Fonts: **Manrope** (400–800) + **Cinzel** (600–700) already enqueued via Google Fonts in `functions.php`. Article Template also needs **Lora** (400, 500, italic 400) — add it to the same Google Fonts `wp_enqueue_style` URL when building that template.
...
- Next up: **Article page template**, see spec below.
...
2. **"Verified the fix is live (raw `fetch()` shows new content) but the Browser tool still shows old behavior."** The Browser tool's own page navigation can render a stale cached DOM/CSS even when a `fetch(url, {cache:'no-store'})` call to the same URL returns fresh content. Don't trust a single `navigate` + `read_page` check as proof of "still broken" — open a brand-new tab, or fetch the exact final (post-redirect, versioned) URL directly, or just ask the user to hard-refresh in their own real browser.
...
## Article Template — design spec (next up)
...
## TODO
- [x] Header/nav (PR #1)
- [ ] Article page template (single-post.php or a new template — see spec above)
